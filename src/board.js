...
class Board {
    constructor() {
        ...
        this.bb = BigInt(0)
        this.pieceBoards = {   'k': BigInt(0), 'q': BigInt(0), 'r': BigInt(0), 'b': BigInt(0), 'n': BigInt(0), 'p': BigInt(0),
                               'K': BigInt(0), 'Q': BigInt(0), 'R': BigInt(0), 'B': BigInt(0), 'N': BigInt(0), 'P': BigInt(0)}
    ...
    parseFenToBoard(fen) {
        ...
        let fen_index = 56 // fens start at a8
        let ranksRead = 1
        let whiteSpace = 0
        ...
        for (let i = 0; i < fen.length; i++) {
            if (ranksRead == 8 && whiteSpace == 1) {
                ...
                if ('kqrbnpKQRBNP'.includes(fen[i])) {
                    this.pieceBoards[fen[i]] = BitHelper.setBit(this.pieceBoards[fen[i]], fen_index)
                    fen_index += 1
                }
                
                if ('12345678'.includes(fen[i])) {
                    fen_index += parseInt(fen[i])
                }

                if (fen[i] == '/') {
                    fen_index = (56 - (ranksRead * 8))
                    ranksRead += 1
                }
    ...
    flipBoard() {
        for (let sq = 0; sq < 32; sq++) {
            for (let [piece, board] of Object.entries(this.pieceBoards)) {
                let bit = BitHelper.getBit(board, sq)
                let inverted_bit = BitHelper.getBit(board, sq ^ 56)
                
                board = BitHelper.updateBit(board, sq, inverted_bit)
